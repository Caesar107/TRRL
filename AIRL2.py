...
def run_airl_breakout_test():
    ...
    # Step 2: 训练专家策略
    print("Training a PPO expert...")
    expert_policy = train_expert(env)

    # 评估专家策略
    expert_reward_mean, expert_reward_std = evaluate_policy(expert_policy, env, n_eval_episodes=5)
    print(f"Expert Policy Reward: {expert_reward_mean} ± {expert_reward_std}")
    ...
    #gen_policy.set_logger(custom_logger)  # 设置生成器的日志记录器

    # 使用 CnnRewardNet 初始化奖励网络
    reward_net = CnnRewardNet(env.observation_space, env.action_space).to(device)
    ...
    airl = AIRL(
        ...
        #logger=custom_logger  # 设置 AIRL 的日志记录器
    )
    ...
    gen_policy.policy.to(device)

    for step in tqdm(range(0, max_timesteps, eval_frequency), desc="Training Progress"):
        airl.train(
            total_timesteps=eval_frequency,
            #gen_train_timesteps=eval_frequency  # 增加生成器的训练步数
        )
        ...
        # 计算和记录 reward
        reward_mean, reward_std = evaluate_policy(gen_policy, env, n_eval_episodes=5)
